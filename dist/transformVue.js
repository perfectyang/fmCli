...
function parseJsSyntax(source, rule) {
    ...
    const { code } = (0, transformJs_1.default)(source, {
        rule: Object.assign(Object.assign({}, rule), { caller: '', importDeclaration: '' }),
...
// 判断表达式是否已经转换成i18n
function hasTransformed(code, functionName) {
...
function handleTemplate(code, rule) {
    ...
    function getReplaceValue(value, isAttribute) {
        value = (0, escapeQuotes_1.escapeQuotes)(value);
        // 表达式结构 $t('xx')
        let expression = `${functionName}('${customizeKey(value, collector_1.default.getCurrentCollectorPath())}')`;
...
function handleScript(source, rule) {
    ...
    if (startIndex !== -1) {
        ...
        const notDefaultCode = (0, transformJs_1.default)(notDefaultPart, Object.assign(Object.assign({}, transformOptions), { rule: stateManager_1.default.getToolConfig().rules.js })).code;
        if (notDefaultCode) {
            return '\n' + notDefaultCode + '\n' + defaultCode + '\n';
        }
        else {
...
function mergeCode(templateCode, scriptCode, stylesCode) {
    return templateCode + '\n' + scriptCode + '\n' + stylesCode;
}
