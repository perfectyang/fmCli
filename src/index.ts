...
// 注册方法
const registerMethods = (methods: string[]) => {
...
program.command("loadExcel").action(() => {
...
function enhanceErrorMessages() {
  ...
  (program as CMD).Command.prototype["unknownOption"] = function (
    ...options: any
  ) {
    ...
    console.log();
    console.log(
      `  ` + chalk.red(`Unknown option ${chalk.yellow(unknownOption)}.`)
    );
...
function suggestCommands(unknownOption: string) {
  ...
  availableOptions.forEach((name) => {
    const isBestMatch =
      leven(name, unknownOption) < leven(suggestion || "", unknownOption);
