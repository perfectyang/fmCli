...
| --localePath      | String  | './locales/zh.json' | 指定提取的中文语言包所存放的路径。                                                     |
| -v,--verbose      | Boolean | false               | 控制台打印更多调试信息                                                                 |
| -h,--help         | Boolean | false               | 查看指令用法                                                                           |
| --skip-extract    | Boolean | false               | 跳过 i18n 转换阶段。                                                                   |
| --locales         | Array   | ['en']              | 根据中文语言包自动翻译成其他语言。用法例子 --locales en zh-CHT                         |
...
2. 进行 i18n 转换，并将提取到的 key-value 提取到中文语言包
...
function Example() {
  const [msg, setMsg] = useState(ts("你好"));
  return (
    <div>
      <p title={ts("标题")}>{msg + ts("呵呵")}</p>
      <button onClick={() => setMsg(msg + ts("啊"))}>{ts("点击")}</button>
    </div>
  );
}
export default Example;
